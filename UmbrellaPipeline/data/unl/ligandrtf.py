...
def is_symmetric(graph, subgraph, nbunch):
    ...
    for n in nbunch:
        ...
        if dist.has_key(distance):
            dist[distance].append(n)
        else:
            dist[distance] = [n]
        # reject cyclohexane
        if graph.degree(n) > 3:
    ...
    for d, nodes in dist.items():
        ...
        for n in nodes:
            if sub == None:
                sub = set([x[0] for x in subgraph.neighbors(n)])
            else:
                if sub != set([x[0] for x in subgraph.neighbors(n)]):
                    return False
...
def find_all_tert_sym_groups(graph):
    ...
    for match in gm.subgraph_isomorphisms_iter():
        ...
        if len(set([x[0] for x in match.keys()])) > 1:
            continue

        # atoms has to be terminal
        flip = dict([(v, k) for (k, v) in match.iteritems()])
        if terts.has_key(flip[1]):
            continue
        if len(_graph[flip[2]]) > 1:
            continue
        if len(_graph[flip[3]]) > 1:
            continue
        if len(_graph[flip[4]]) > 1:
            continue
...
if __name__ == "__main__":
    ...
    for cycle in cycles:
        ...
        dihe2 = root
        for n in cycle[root]:
            if cycle.degree(n) != G.degree(n):
                dihe3 = n
        for n in cycle[root]:
            if cycle.degree(n) == 1:
                continue
            if n not in [dihe2, dihe3]:
                dihe1 = n
        if dihe3 != None:
            for n in G[dihe3]:
                if G.degree(n) == 1:
    ...
    for root, tert in terts.items():
        ...
        try:
            dihe4 = [x for x in G[dihe3].keys() if x != dihe2 and x[0] != "H"].pop()
        except:
            continue
        dihe_str = "set diheatom%d = %s %s %s %s" % (i, dihe1, dihe2, dihe3, dihe4)
        dihe.append(dihe_str)
    ...
    fp.write(
        """* dihedral setup for symmetric unit
*
